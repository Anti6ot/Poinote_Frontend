...
import { Plus, Trash2, ChevronLeft, Search, ChevronRight, Settings, LogOut, ChevronDown } from 'lucide-react';
...
const PageTreeItem = ({ pageId, depth }: PageTreeItemProps) => {
  const { pages, activePageId, addPage, deletePage, setActivePageId } = useWorkspaceCtx();
...
const Sidebar = () => {
  const { pages, addPage, workspaces, activeWorkspaceId, setActiveWorkspaceId, loadWorkspaces } = useWorkspaceCtx();
  ...
  return (
    <aside className="w-60 h-screen flex flex-col bg-notion-sidebar border-r border-border flex-shrink-0 animate-slide-in">
      ...
      <div className="flex items-center justify-between px-3 pt-3 pb-1">
        <button onClick={() => setShowWsSwitcher(!showWsSwitcher)} className="flex items-center gap-1 text-sm font-semibold text-foreground tracking-tight hover:bg-notion-sidebar-hover rounded px-1 py-0.5 transition-colors">
          {activeWs?.name || 'Workspace'}
          <ChevronDown size={13} />
        </button>
        <div className="flex items-center gap-0.5">
          <button onClick={() => setShowSettings(true)} className="p-1 rounded hover:bg-notion-sidebar-hover text-notion-icon hover:text-notion-icon-hover transition-colors">
            <Settings size={14} />
          </button>
        </div>
      </div>
      ...
      {/* Workspace switcher */}
      {showWsSwitcher && (
        <div className="mx-2 mb-1 p-1 bg-background border border-border rounded-md shadow-md">
          {workspaces.map(ws => (
            <button
              key={ws.id}
              onClick={() => { setActiveWorkspaceId(ws.id); setShowWsSwitcher(false); }}
              className={`w-full text-left text-sm px-2 py-1.5 rounded transition-colors ${ws.id === activeWorkspaceId ? 'bg-accent font-medium' : 'hover:bg-accent/50'}`}
            >
              {ws.name}
            </button>
          ))}
      ...
      {showSettings && activeWs && (
        <WorkspaceSettings workspace={activeWs} onClose={() => setShowSettings(false)} onUpdated={loadWorkspaces} />
      )}
    </aside>
  );
};
